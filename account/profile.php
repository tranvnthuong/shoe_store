...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
    ...
    $stmt = $conn->prepare("UPDATE users SET password=?, phone=?, address=? WHERE email=?");
    $stmt->bind_param("ssss", $day_of_birth, $phone, $address, $email);
    if ($stmt->execute()) {
  ...
  <div class="card shadow p-4">
    ...
    <form method="POST">
      ...
      <div class="mb-3">
        ...
        <input type="date" name="day_of_birth" value="<?= $user['day_of_birth'] ?>" class="form-control" required>
      </div>
      <div class="mb-3">
